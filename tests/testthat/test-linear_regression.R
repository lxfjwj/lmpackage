...
test_that("", {
  ...
  expect_equal(linear_regression(y~x,data, offset = off)$residuals, unname(testresult$residuals))
  expect_equal(linear_regression(y~x,data, offset = off)$effects, unname(testresult$effects))
  expect_equal(linear_regression(y~x,data, offset = off)$df.residual, unname(testresult$df.residual))
})
